...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'get') {
      ...
      const cur = await db.getById('os_records', id);
    ...
    if (action === 'update') {
      ...
      for (const k of ['title', 'status', 'amount', 'customer_id', 'customer_name', 'due_at']) {
      ...
      const item = await db.update('os_records', id, patch);
      return res.status(200).json({ item: item || Object.assign({}, cur, patch) });
    }
    if (action === 'delete') {
      await db.remove('os_records', id);
