...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    await db.insert('downloads', { slug, title: resource.title || slug, email: em, name: name || null, created_at: new Date().toISOString() });
    // capture
    if (!(await db.findOne('leads', 'email', em))) {
      await db.insert('leads', { email: em, owner_name: name || null, source: `download:${slug}`, status: 'new', score: 'warm', unsubscribed: false, created_at: new Date().toISOString() });
    }
