...
(function(){
  ...
  // Ensure TMI's social links sit in the page footer. Injects a row of icon
  // links into the footer's inner container, inheriting the footer's own text
  // color so it reads correctly on both light and dark footers. Skips any footer
  // that already carries social links (the main brand pages bake them in).
  function ensureFooterSocial(){
    var foot=document.querySelector('footer'); if(!foot) return;
    if(foot.querySelector('[data-tmi-social]')) return;                       // already baked in
    if(foot.querySelector('a[href*="instagram.com/tmitechai"]')) return;      // already present some other way
    var host=foot.querySelector('.wrap, .container, .foot-inner, .footer-inner') || foot.firstElementChild || foot;
    var SOC=[
      ['https://www.youtube.com/@tmitechai','TMI on YouTube','M23.5 6.19a3.02 3.02 0 0 0-2.12-2.14C19.5 3.55 12 3.55 12 3.55s-7.5 0-9.38.5A3.02 3.02 0 0 0 .5 6.19 31.6 31.6 0 0 0 0 12a31.6 31.6 0 0 0 .5 5.81 3.02 3.02 0 0 0 2.12 2.14c1.88.5 9.38.5 9.38.5s7.5 0 9.38-.5a3.02 3.02 0 0 0 2.12-2.14A31.6 31.6 0 0 0 24 12a31.6 31.6 0 0 0-.5-5.81zM9.6 15.6V8.4L15.82 12z'],
      ['https://www.instagram.com/tmitechai','TMI on Instagram','M12 2.16c3.2 0 3.58.01 4.85.07 1.17.05 1.8.25 2.23.42.56.21.96.47 1.38.9.43.42.69.82.9 1.38.17.43.37 1.06.42 2.23.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.05 1.17-.25 1.8-.42 2.23-.21.56-.47.96-.9 1.38-.42.43-.82.69-1.38.9-.43.17-1.06.37-2.23.42-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-1.17-.05-1.8-.25-2.23-.42a3.72 3.72 0 0 1-1.38-.9 3.72 3.72 0 0 1-.9-1.38c-.17-.43-.37-1.06-.42-2.23-.06-1.27-.07-1.65-.07-4.85s.01-3.58.07-4.85c.05-1.17.25-1.8.42-2.23.21-.56.47-.96.9-1.38.42-.43.82-.69 1.38-.9.43-.17 1.06-.37 2.23-.42C8.42 2.17 8.8 2.16 12 2.16zM12 0C8.74 0 8.33.01 7.05.07 5.78.13 4.9.33 4.14.63a5.88 5.88 0 0 0-2.13 1.38A5.88 5.88 0 0 0 .63 4.14C.33 4.9.13 5.78.07 7.05.01 8.33 0 8.74 0 12s.01 3.67.07 4.95c.06 1.27.26 2.15.56 2.91.31.79.72 1.46 1.38 2.13.67.66 1.34 1.07 2.13 1.38.76.3 1.64.5 2.91.56C8.33 23.99 8.74 24 12 24s3.67-.01 4.95-.07c1.27-.06 2.15-.26 2.91-.56a5.88 5.88 0 0 0 2.13-1.38 5.88 5.88 0 0 0 1.38-2.13c.3-.76.5-1.64.56-2.91.06-1.28.07-1.69.07-4.95s-.01-3.67-.07-4.95c-.06-1.27-.26-2.15-.56-2.91a5.88 5.88 0 0 0-1.38-2.13A5.88 5.88 0 0 0 19.86.63c-.76-.3-1.64-.5-2.91-.56C15.67.01 15.26 0 12 0zm0 5.84A6.16 6.16 0 1 0 18.16 12 6.16 6.16 0 0 0 12 5.84zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.41-11.85a1.44 1.44 0 1 0 0 2.88 1.44 1.44 0 0 0 0-2.88z'],
      ['https://www.linkedin.com/company/tmiai/','TMI on LinkedIn','M20.45 20.45h-3.56v-5.57c0-1.33-.02-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05c.48-.9 1.63-1.85 3.36-1.85 3.6 0 4.27 2.37 4.27 5.45v6.29zM5.34 7.43a2.07 2.07 0 1 1 0-4.14 2.07 2.07 0 0 1 0 4.14zM7.12 20.45H3.56V9h3.56v11.45zM22.22 0H1.77C.79 0 0 .77 0 1.73v20.54C0 23.23.79 24 1.77 24h20.45c.98 0 1.78-.77 1.78-1.73V1.73C24 .77 23.2 0 22.22 0z'],
      ['https://www.facebook.com/share/19Hiw8KsUq/?mibextid=wwXIfr','TMI on Facebook','M24 12.07C24 5.41 18.63 0 12 0S0 5.41 0 12.07C0 18.1 4.39 23.09 10.13 24v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.68.24 2.68.24v2.97h-1.51c-1.49 0-1.96.93-1.96 1.89v2.25h3.33l-.53 3.49h-2.8V24C19.61 23.09 24 18.1 24 12.07z']
    ];
    var row=mk('div',''); row.setAttribute('data-tmi-social','');
    row.style.cssText='display:flex;gap:14px;align-items:center;margin-top:26px;padding-top:20px;border-top:1px solid rgba(128,128,128,0.18);flex-wrap:wrap;';
    SOC.forEach(function(s){
      var a=mk('a','','<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="'+s[2]+'"/></svg>');
      a.href=s[0]; a.target='_blank'; a.rel='noopener'; a.setAttribute('aria-label',s[1]);
      a.style.cssText='display:inline-flex;color:inherit;opacity:0.5;transition:opacity .15s;';
      a.addEventListener('mouseenter',function(){a.style.opacity='1';});
      a.addEventListener('mouseleave',function(){a.style.opacity='0.5';});
      row.appendChild(a);
    });
    host.appendChild(row);
  }

  function init(){ ensureExtras(); ensureFooterTapMe(); ensureFooterBook(); ensureFooterSocial(); styleFotfLetters(); enhanceSideNav(); enhanceDrawerLike(document.getElementById('drawer')); enhanceDesktop(); enhanceTopnav(); enhanceArticleHeader(); railToggle(); }
